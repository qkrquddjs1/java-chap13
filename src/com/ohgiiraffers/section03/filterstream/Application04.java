...
public class Application04 {
    public static void main(String[] args) {
        /*  ObjectInputStream / ObjectOutputStream : 객체 단위 입출력 이름
        객체 단위 입출력 보조스트림을 이해하고 사용할 수 있다. */
        ...
        /* DTO 클래스에 직렬화 처리를 하지 않았다면 실행시 java,io.NotSerializableException이 발생한다.
         * 직력화란?
         *  자바 시스템 내부에서 사용되는 객체 또는 데이터를 외부에서도 사용 할 수 있도록 byte 형태로
         *  데이터를 변환하는 기술을 말한다.
         *  반대로 바이트로 변환 된 데이터를 다시 객체로 변환하는 길수을 역직렬화라고 한다
         * */
        ...
        try (ObjectInputStream objIn = new ObjectInputStream(new FileInputStream("testObjectStream.txt"))){
            ...
            /* readObject() 메소드는 역직렬화 해서 데이터를 객체화 할떄 해당하는 클래스가 없을 가능성이 있으므로
        ...
        for(MemberDTO meber : inputMembers) {
